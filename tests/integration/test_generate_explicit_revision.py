...
def _project_root_with_older_and_newer_revision(tmp_path: Path) -> Path:
    ...
    original = root / "editions" / "11e" / "2026-06-01-00"
    older = root / "editions" / "11e" / "2026-05-01-00"
...
def test_explicit_revision_selects_exact_older_revision_not_latest(tmp_path):
    ...
    doc = generate(config, _paths(root), "11e", revision=RevisionId.parse("2026-05-01-00"), language="en")[0]

    assert str(doc.revision) == "2026-05-01-00"
    assert doc.html_path == root / "out" / "11e" / "2026-05-01-00" / "en.html"
